...
class ImageDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, 
        ...
        if self.train:
            ...
            data, label = self.getValDataInfo(os.path.join(root, "val"))
        ...
        if self.train:
            ...
            if os.path.exists(noise_file):
                ...
                data_idn_new = torch.from_numpy(resized_img).float()
                targets = torch.from_numpy(np.array(label))
                dataset = zip(data_idn_new, targets)
    ...
    def getValDataInfo(self, root):
        ...
        for file in files:
            if file in label_dict:
                data_path = os.path.join(image_dir, file)
                data.append(data_path)
                label.append(class_to_idx[label_dict[file]])
