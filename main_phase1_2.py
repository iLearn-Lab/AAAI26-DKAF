...
parser.add_argument("--noise_mode", choices=['sym', 'idn'], default='sym')
parser.add_argument("--noise_ratio", default='0.2')
...
def train(epoch, dataloader, M, vis_proto):
    ...
    for batch_idx, (inputs, targets, comple_targets, index) in enumerate(dataloader):
        ...
        if epoch > cfg.warmup:
            ...
            probs_mix = mix_alpha * probs_text + (1 - mix_alpha)
        ...
        for i in range(neg_outputs.shape[0]):
            ...
            with torch.no_grad():
                if epoch <= (cfg.warmup+2):
                    idx_select_i = (p_clean_i > 0.5).cpu()
                else:
                    momentum_ema = 0.7
        ...
        idx_select = (p_clean_ema[index] > 0.5).cpu()
        ...
        if epoch <= cfg.warmup:
            correct += pseudo.eq(clean_label[index]).cpu().sum().item()
        else:
            correct += pseudo_mix.eq(clean_label[index]).cpu().sum().item()
    ...
    acc, prec, rec = 100.*correct/total, 100.*precision/num_iter, 100.*recall/num_iter
    f1 = 2 * prec * rec / (prec + rec)
    print("\n| Train Epoch #%d\t Accuracy: %.2f%%\t Precision: %.2f%%\t Recall: %.2f%%\t F1: %.2f%%\t" % (epoch, acc, prec, rec, f1))
    
    if epoch > (cfg.warmup+1):
        mix_alpha = text_correct_num / (text_correct_num + vision_correct_num + 1e-6)

    if epoch == cfg.epochs:
        return acc, prec, rec, high_conf_list, p_logits_dict
    else:
        return acc, prec, rec, high_conf_list, None
...
if cfg.dataset.startswith("cifar"):
    ...
    loader = dataloader.cifar_dataloader(cfg.dataset, noise_mode=cfg.noise_mode, noise_ratio=cfg.noise_ratio,\
                                        batch_size=cfg.batch_size, num_workers=cfg.num_workers, root_dir=cfg.data_path, model=cfg.model, seed=cfg.seed)
    ...
    from dataloader import dataloader_general as dataloader
    train_loader, _, test_loader = dataloader.build_loader(cfg)
...
num_samples = len(noise_label)
num_noisy_samples = (noise_label != clean_label).sum().item()
noise_rate = num_noisy_samples / num_samples
print(f"Noise mode: {cfg.noise_mode}")
print(f"Noise rate: {noise_rate:.4f}")
...
def compute_global_similarity_on_clean(model, loader, total_pseudos):
    ...
    S_txt = T @ T.t()
    global mix_alpha
    S = mix_alpha * S_txt + (1-mix_alpha) * S_vis
    return S, vis_proto, txt_proto
...
for epoch in range(1, cfg.epochs + 1):
    ...
    if epoch >= cfg.warmup:
        ...
        for sample_id, label in high_conf_list:
            sigma[label] += 1 
        S, vis_proto, txt_proto = compute_global_similarity_on_clean(model, train_loader, total_pseudos)
        M = compute_margin_matrix(S, sigma, m_scale=9)
        txt_proto_last = txt_proto
    ...
    if epoch == cfg.epochs:
        ...
        refined_samples = []
        model.eval()
        txt_proto_norm = F.normalize(txt_proto_last.detach(), dim=-1, eps=1e-6)

        with torch.no_grad():
            for batch_idx, (inputs, targets, comple_targets, index) in enumerate(train_loader):
                inputs = inputs.cuda()
                
                logits_text, _, neg_text_features, feats = model(inputs, return_neg=True)
                ...
                for i in range(inputs.shape[0]):
                    ...
                    feats_proj = F.normalize(feats[i].detach() @ model.image_encoder.proj.detach(), dim=-1, eps=1e-6)
                    sim_pos = torch.matmul(txt_proto_norm[pred_class_mix].detach(), feats_proj.detach())
                    
                    sim_neg = 0
                    for j in range(neg_text_features.shape[0]):
                        sim_neg += torch.matmul(neg_text_features[j][pred_class_mix].detach(), feats_proj.detach())
                    sim_neg /= neg_text_features.shape[0]
