...
def train(epoch, dataloader, lambda_2):
    ...
    for batch_idx, (inputs, targets, _, index) in enumerate(dataloader):
        ...
        if clean_mask.sum() > 0:
            loss_clean = criterion(outputs[clean_mask], targets[clean_mask]).mean()
        ...
        if epoch >= 3:
            if high_conf_mask.sum() > 0:
                ...
                tars_clamped = torch.clamp(soft_targets, min=1e-5) 
                log_targets = torch.log(tars_clamped)
                
                preds = torch.exp(log_preds)
                preds_clamped = torch.clamp(preds, min=1e-5)
...
def load_samples(dataset: str, noise_mode: str, noise_ratio: float, base_dir: str = "./phase1_2"):
    ...
    refined_samples = torch.load(refined_samples_path)
...
refined_samples = load_samples(cfg.dataset, cfg.noise_mode, cfg.noise_ratio, base_dir="./phase1_2")
refined_samples_dict = {sample_id: pseudo_label for sample_id, pseudo_label in refined_samples}

best_acc = 0

sum1 = total_clean_idx.sum().item()
sum2 = len(refined_samples)
lambda_2 = sum2 / (sum2 + sum1)
