...
class Model(nn.Module):
    def __init__(self, cfg, clip_model, text_features=None):
        ...
        self.tuner = ViT_Tuner(cfg, clip_model, text_features=text_features) # VPT
        # text encoder
        self.prompt_learner = PromptLearner(cfg, clip_model) # prompt tuning
    ...
    def forward(self, image, return_sim=False, return_neg=False):
        ...
        neg_text_features = F.normalize(neg_text_features, dim=-1, eps=1e-6)
        logit_scale = self.logit_scale.exp()

        if return_neg:
            logits_pos = logit_scale * image_features @ text_features.t()
            num_neg = neg_text_features.shape[0]
            logits_neg_list = []
            for i in range(num_neg):
               logits_neg_i = logit_scale * image_features.detach() @ neg_text_features[i].t()
               logits_neg_list.append(logits_neg_i)
            logits_neg = torch.stack(logits_neg_list, dim=0)
            return logits_pos, logits_neg, neg_text_features, feats
        elif return_sim:
            logits_pos = logit_scale * image_features @ text_features.t()
            return logits_pos
        else:
