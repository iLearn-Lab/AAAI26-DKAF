...
class TextEncoder(nn.Module):
    ...
    def forward(self, prompts, tokenized_prompts):
        ...
        neg_prompts = prompts[1]  # (num_neg, n_cls, prompt_len, dim)
        neg_tokenized_prompts = tokenized_prompts[1]  # (num_neg, n_cls, prompt_len)

        num_neg = neg_prompts.shape[0]
        neg_txt_features = []

        for i in range(num_neg):
            neg_feat = self.forward_one_prompt(neg_prompts[i], neg_tokenized_prompts[i])  # (n_cls, dim)
            neg_txt_features.append(neg_feat)  # list of (n_cls, dim)

        neg_txt_features = torch.stack(neg_txt_features, dim=0)  # (num_neg, n_cls, dim)
...
class PromptLearner(nn.Module):
    def __init__(self, cfg, clip_model):
        ...
        neg_ctx_vectors = torch.empty(num_negatives, n_ctx, ctx_dim, dtype=dtype)
    ...
    def forward(self):
        ...
        neg_prompts = []
        for i in range(self.neg_ctx.shape[0]):
            neg_prompt_i = self.forward_one_prompt(self.neg_ctx[i])
            neg_prompts.append(neg_prompt_i)
        neg_prompts = torch.stack(neg_prompts, dim=0)
